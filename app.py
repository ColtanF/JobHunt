...
# Change this to w/e your db password is
...
def index():
    ...
    if 'logged_in' in session:
        ...
        result = cur.execute(
            "SELECT * FROM jobs_tbl WHERE (username = %s AND rejected = False)", [session["username"]])
...
@isLoggedIn
def rejects():
    # Retrieve jobs postings from which I've been rejected.
    cur = mysql.connection.cursor()

    # Grab the jobs where rejected=True
    result = cur.execute(
        "SELECT * FROM jobs_tbl WHERE (username = %s and rejected = TRUE)", [session["username"]])
    jobs = cur.fetchall()
    cur.close()
    return render_template('jobs.html', jobs=jobs, rejects=True)
...
class JobForm(Form):
    ...
    companyInfo = TextAreaField('Company Description', [
                                validators.Length(min=1)])
    positionInfo = TextAreaField('Position Description', [
                                 validators.Length(min=1)])
...
def add_job():
    ...
    if request.method == "POST" and form.validate():
        ...
        cur.execute("INSERT INTO jobs_tbl(company, position, companyInfo, positionInfo, reqsIMeet, reqsIDontMeet, salary, address, links, status, username, rating, rejected) VALUES(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, 0, false)",
...
@app.route('/rejected/<string:id>', methods=["POST"])
@isLoggedIn
def rejected(id):
    # Set rejection flag to true in the db
    cur = mysql.connection.cursor()
    result = cur.execute(
        "UPDATE jobs_tbl SET rejected = True WHERE id = %s", [id])
    mysql.connection.commit()
    cur.close()

    flash("Job moved to rejected. You'll get 'em next time!", "success")
    return redirect(url_for("jobs"))
...
def view_job(id):
    ...
    if request.method == "POST":
        ...
        if ratingsForm['stars']:
            cur = mysql.connection.cursor()
            result = cur.execute("UPDATE jobs_tbl SET rating = %s WHERE id = %s", [
                                 ratingsForm['stars'], id])
            mysql.connection.commit()
            result = cur.execute("SELECT * FROM jobs_tbl WHERE id = %s", [id])
            job = cur.fetchone()
            cur.close()
...
def register():
    ...
    if request.method == "POST" and form.validate():
        ...
        data = cur.fetchall()
        if len(data) != 0:
            flash("Account with username " + username +
                  " already exists, please choose a different username.", "warning")
